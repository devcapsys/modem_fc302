...
def run_step(log, config: configuration.AppConfig, update_percentage=lambda x: None):
    ...
    min = config.configItems.measure_ac_vrms_2.min
    max = config.configItems.measure_ac_vrms_2.max
    
    if min is None or max is None:
        return_msg["infos"].append("Les valeurs min/max pour measure_ac_vrms_2 ne sont pas définies.")
        return 1, return_msg
    
    config.modem_fc302_manager.send_command_Cr("AT+TEST=TX01")
    time.sleep(2)  # Wait for the rigol to stabilize
    
    command = ":MEASure:ITEM? VRMS,CHANnel1"
    measure = config.oscilloscope_rigol_manager.send_command(command)
    
    if measure is None:
        return_msg["infos"].append("Erreur lors de la mesure AC.Vrms 2.")
        return 1, return_msg
    
    log(f"Mesure AC.Vrms 2 : {measure}V, min={min}, max={max}", "blue")
    id = config.save_value(step_name_id, "AC_Vrms_2", float(measure), "V", min_value=min, max_value=max)
    ...
    if measure_float < min_float or measure_float > max_float:
        return_msg["infos"].append(f"Mesure AC.Vrms 2 hors tolérance : {measure}V, min={min}, max={max}")
        return 1, return_msg
